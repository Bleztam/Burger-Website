...
export function AiFlavorPredictor() {
  ...
  return (
    <section id="combo" className="relative py-24 px-4 bg-neutral-950 border-t border-b border-neutral-900 overflow-hidden">
      ...
      <div className="max-w-md mx-auto relative min-h-[350px]">
        ...
        {step < 4 && (
          <div className="w-full bg-neutral-900 h-1.5 rounded-full mb-6 overflow-hidden relative">
            <div 
        ...
        {step < 4 && (
          <div 
            ref={cardRef}
            className="bg-neutral-900 border border-neutral-850 p-6 rounded-2xl shadow-2xl relative"
          >
                  ...
                  {moods.map((item) => (
                    <button
                      ...
                      className={`p-4 rounded-xl border text-left transition-all duration-300 group cursor-pointer relative overflow-hidden ${
                        mood === item.name
                          ? 'border-amber-500 bg-neutral-850'
                          : 'border-neutral-800 bg-neutral-950 hover:border-neutral-750'
                      }`}
                    >
                      <span className="relative z-10 text-xs font-bold uppercase tracking-wider text-neutral-300">
                        {item.label}
                      </span>
                      {mood === item.name && (
            ...
            {step === 2 && (
              <div>
                  ...
                  {cravings.map((item) => (
                    <button
                      ...
                      className={`w-full p-4 rounded-xl border text-left transition-all duration-300 group cursor-pointer flex justify-between items-center ${
                        craving === item.name
                          ? 'border-amber-500 bg-neutral-850 shadow-[0_0_15px_rgba(245,158,11,0.05)]'
                          : 'border-neutral-800 bg-neutral-950 hover:border-neutral-750'
                      }`}
                    >
                      <div>
                        <span className="text-xs font-bold uppercase tracking-wider block text-neutral-200 mb-0.5">
                          {item.label}
                        </span>
                        <span className="text-[10px] text-neutral-450">{item.desc}</span>
                      </div>
                      {craving === item.name && (
                ...
                <div className="mt-8 flex justify-between">
                  <button
                    onClick={() => setStep(1)}
                    className="px-5 py-2.5 rounded-xl border border-neutral-800 hover:bg-neutral-850 text-neutral-300 font-semibold text-xs tracking-wider uppercase cursor-pointer"
                  >
                    Back
                  </button>
                  <button
        ...
        {loading && (
          <div className="absolute inset-0 bg-neutral-950/85 backdrop-blur-sm z-30 flex flex-col items-center justify-center border border-neutral-850 rounded-2xl">
            <Flame className="text-amber-500 animate-bounce mb-3" size={32} />
            <h4 className="font-display text-lg uppercase text-neutral-100 tracking-wider">Taste Algorithm Processing...</h4>
            <p className="text-[10px] text-neutral-450 mt-1">Configuring protein/lipid coordinate pathways</p>
          </div>
        )}
        ...
        {step === 4 && recommendation && (
          <div 
            ref={revealCardRef}
            className="bg-neutral-900 border-2 border-amber-500 p-6 rounded-2xl shadow-2xl relative flex flex-col items-center text-center opacity-0"
          ...
          >
            ...
            <h3 className="font-display text-2xl uppercase tracking-wider text-white mb-2 leading-none">
              {recommendation.burger.name} + {recommendation.drink.name}
            </h3>
            
            <p className="text-xs text-neutral-400 max-w-sm mb-6 leading-relaxed bg-neutral-950/60 p-4 rounded-xl border border-neutral-850 text-left italic">
            ...
            <div className="w-full flex flex-col gap-2">
              ...
              <button
                onClick={handleReset}
                className="w-full py-2.5 rounded-xl bg-neutral-950 border border-neutral-800 hover:bg-neutral-900 text-neutral-350 font-semibold uppercase tracking-wider text-[10px] flex items-center justify-center gap-1.5 cursor-pointer"
              >
